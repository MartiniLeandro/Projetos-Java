...
public class PlanoService {
    ...
    public Plano findById(Long id){
        return planoRepository.findById(id).orElseThrow(() -> new NotFoundException("Não existe plano com este ID"));
    }
    ...
    public PlanoResponseDTO create(PlanoRequestDTO data){
        if(data.valorBase().signum() < 0) throw new ArithmeticException("O valor não pode ser negativo");
    ...
    public PlanoResponseDTO update(PlanoRequestDTO data, Long id){
        Plano updatedPlano = planoRepository.findById(id).orElseThrow(() -> new NotFoundException("Não existe plano com este Id"));
        ...
        }else {throw new ArithmeticException("O valor não pode ser negativo ou nulo");}
    ...
    public void inativar(Long id){
        Plano plano = planoRepository.findById(id).orElseThrow(() -> new NotFoundException("Não existe plano com este Id"));
