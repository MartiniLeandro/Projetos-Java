...
public interface AlunoRepository extends JpaRepository<Aluno,Long> {
    ...
    @Query(value = "SELECT m.mes AS mes, (SELECT COUNT(id) FROM alunos WHERE EXTRACT(YEAR FROM data_cadastro) = :ano AND EXTRACT(MONTH FROM data_cadastro) = m.mes) AS novosAlunos, (SELECT COUNT(DISTINCT al.id) FROM alunos al INNER JOIN contratos co ON al.id = co.aluno_id WHERE al.status = 'ATIVO' AND co.status = 'ATIVO' AND (EXTRACT(YEAR FROM co.data_inicio) * 12 + EXTRACT(MONTH FROM co.data_inicio)) <= (:ano * 12 + m.mes) AND (EXTRACT(YEAR FROM co.data_fim) * 12 + EXTRACT(MONTH FROM co.data_fim)) >= (:ano * 12 + m.mes)) AS alunosAtivos FROM generate_series(1, :mesLimite) AS m(mes) ORDER BY m.mes ASC", nativeQuery = true)
