...
public interface ContratoRepository extends JpaRepository<Contrato,Long> {
    ...
    @Query(value = "select count(co.id) as contratosAtivos from contratos co inner join planos pl on co.plano_id = pl.id where (:categoria is null or pl.categoria ilike concat('%', :categoria, '%')) and co.status = 'ATIVO' and co.data_fim between current_date and (current_date + CAST(:diasAlerta AS int))", nativeQuery = true)
