...
public interface LaunchRepository extends JpaRepository<Launch,Long> {
    ...
    @Query(value = "select l.* from launches as l join categories as c on l.category_id = c.id where l.user_id = :user_id and (:type_value is null or c.type_value = :type_value) and (:category_id is null or c.id = :category_id) and (cast(:startDate as date) is null or l.date >= :startDate) and (cast(:endDate as date) is null or l.date <= :endDate)", nativeQuery = true) //adicionar paginação
